...
public class GraphWorker extends Thread {
	...
	public GraphWorker ( final SurfaceHolder surfaceHodler, final GraphManager manager ) {
	...
	private void updateScreen () {
		...
		// TODO: Check, normalement peut être directement intégré dans l'event (? lock unlock attention)
		hitDetection( canvas );
	...
	private void hitDetection ( Canvas canvas ) {
		...
		// event occurred
		if ( tapPosition.equals( 0, 0 ) ) return;
		...
		// can not continue.
		if ( !invertible ) {
			Log.e( "MLV-hitDetection", "A matrix from canvas.getMatrix() is not invertible" );
			return;
		}
