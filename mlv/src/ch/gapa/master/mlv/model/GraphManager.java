...
public final class GraphManager {
  ...
  public void draw ( final Canvas canvas ) {
    synchronized ( LOCK ) {
      ...
      for ( Edge<ArtistWrapper> edge : _graph.getEdges() ) {
        ...
        if ( edge.getStart().isFaded() || edge.getEnd().isFaded() ) _edgePaint.setAlpha( 100 );
        else
  ...
  public void doubleTap ( TapEvent event ) {
  ...
  public void surfaceTap ( final TapEvent event ) {
    synchronized ( LOCK ) {
      if ( current != null && current.contains( event.getLocation() ) ) { // Current
        // node
        // pressed
