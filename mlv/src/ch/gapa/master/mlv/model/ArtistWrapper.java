...
public class ArtistWrapper {
	...
	private void defineColor() {
		Collection<Tag> tags = Artist.getTopTags(_artist.getName(),
				Constants.API_KEY);
	...
	public void draw(final Canvas canvas) {
		...
		canvas.drawText(_artist.getName(), _position.x, _position.y + RADIUS
				+ _paint.getTextSize(), _paintText);
	}
	...
	public boolean contains(final Point location) {
		return (location.x > (_position.x - RADIUS)
				&& location.x < (_position.x + RADIUS)
	...
	/**
	 * Returns whether given objects equals to self ArtistWrapper object.
	 * Override for {@link Graph} purposes.
	 * 
	 * @param obj
